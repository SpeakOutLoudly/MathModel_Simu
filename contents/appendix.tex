...
本论文的所有支撑材料组织在\texttt{Materials/}目录下，具体分类和说明如表\ref{table:supporting_materials}所示：
...
\begin{table}[h!]
    ...
    \centering
    \bicaption[支撑材料分类说明]{支撑材料分类说明。}%
             [Classification of Supporting Materials]{Classification of Supporting Materials.}
    \begin{tabularx}{\textwidth}{p{1.8cm}p{4.2cm}X}
      \toprule
      材料类型 & 文件路径 & 说明 \\
      \midrule
      % ---------------- 实现代码 ----------------
      \multirow{3}{1.8cm}{实现代码} % 共有 4 行，* 表示自适应宽度
        & \texttt{code/run\_optimization.py} & 执行入口脚本 \\
        & \texttt{code/tourism\_optimization.py} & 问题一、问题三主要程序 \\
        & \texttt{code/Q2\_k\_means.py} & 问题二：K-means 算法代码 \\
      \midrule
      % ---------------- 结果材料 ----------------
      \multirow{7}{1.8cm}{结果材料} % 共有 8 行
        & \texttt{终端输出.docx} & \texttt{run\_optimization.py} 终端日志 \\
        & \texttt{Q1/optimization\_result\_费用优先型.txt} & 问题 1：费用优先型解 \\
        & \texttt{Q1/optimization\_result\_均衡型.txt}\newline%
          \texttt{Training\_Loss\_Curve.png} & 问题 1：均衡型解 \\
        & \texttt{Q1/optimization\_result\_时间优先型.txt}\newline%
          \texttt{ΔE2000\_Error\_Histogram.png} & 问题 1：时间优先型解 \\
        & \texttt{Q2/benefit\_vs\_expan.png} & 问题 2：收益-扩张曲线 \\
        & \texttt{Q3/clustering\_results.txt} & 问题 3：聚类结果（文本） \\
        & \texttt{Q3/optimization\_clusters.png} & 问题 3：聚类结果可视化 \\
      \midrule
      说明文档 & \texttt{README.txt} & 项目环境与运行指南 \\
      \bottomrule
    \end{tabularx}
